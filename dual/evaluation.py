...
def compute_vr_func(func, best_sol, idx, is_unittest=False):
    ...
    gen_test_texts = [tc.text for tc in func.generated_testcases]
...
def show_results(functions, tp, fp, tn, fn):
...
def compute_validity_rate(ranked_result, ground_truth_exec_result, functions, is_unittest):
    ...
    for idd, func in enumerate(functions):
        if func.task_id in ranked_result and ranked_result[func.task_id]:
            ...
            print('pppppppppppp')
            try:
                best_sol = list(ground_truth_exec_result[func.task_id].keys())[0]
            except Exception:
                continue
            # print(f'best sol is \n{best_sol}')
