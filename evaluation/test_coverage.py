...
def measure_coverage(functions: List[Function], dataset):
    ...
    for idx, func in tqdm(enumerate(functions)):
        ...
        lines_per_testcase = []
        total_lines = 1
        for test_case in func.generated_testcases:
            if test_case.is_valid == 1 and test_case.prediction_is_valid == 1:
...
if __name__ == '__main__':
    ...
    parser.add_argument("--llm", type=str, required=True, help="Name of LLM model (e.g. gpt-4 or gpt-3.5-turbo)",
                        choices=VALID_LLMS)
